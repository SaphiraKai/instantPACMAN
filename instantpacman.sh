...
*Install)
    ...
    st -e sh -c "(nice yay -S $PACKAGE || bash) && notify-send 'finished installing '$PACKAGE"
    ;;
*Remove)
    PACKAGE="$(instantchoosepackage -i)"
    [ -n "$PACKAGE" ] || exit
    st -e bash -c "(nice yay -R $PACKAGE || bash) && notify-send finished\ uninstalling\ $PACKAGE; sleep 4"
    ;;
*Browse)
    pamac-manager &
    exit
